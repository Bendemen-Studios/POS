...
export default function POSHome() {
  ...
  const handleProductClick = product => { const productVariations = Array.isArray(product.variations_data) && product.variations_data.length ? product.variations_data : (Array.isArray(product.variations) && typeof product.variations[0] === 'object' ? product.variations : []); if (productVariations.length) { setSelectedProductForVariations({ ...product, variations_data: productVariations }); return; } const stock = product.stock_quantity; const price = parseFloat(product.price || 0); if (stock !== null && stock !== undefined && Number(stock) <= 0) { setStockWarningModal({ show: true, product, variation: null, price }); return; } if (product.price === '' || product.price === null || (price === 0 && !product.is_fixed_zero)) { setOpenAmountProduct(product); setCustomPriceInput(''); return; } addToCart(product, price); };
  ...
  const handleSelectVariation = variation => { const price = parseFloat(variation.price || selectedProductForVariations.price || 0); if (variation.stock_quantity !== null && variation.stock_quantity !== undefined && Number(variation.stock_quantity) <= 0) { setSelectedProductForVariations(null); setStockWarningModal({ show: true, product: selectedProductForVariations, variation, price }); return; } const attr = formatAttributes(variation.attributes) || `Variatie #${variation.id}`; addToCartCustom({ ...selectedProductForVariations, id: `${selectedProductForVariations.id}_var_${variation.id}`, product_id: selectedProductForVariations.id, variation_id: variation.id, name: `${selectedProductForVariations.name} - ${attr}`, price }); setSelectedProductForVariations(null); };
  const handleConfirmStockWarning = () => { const { product, variation, price } = stockWarningModal; if (variation) { const attr = formatAttributes(variation.attributes) || `Variatie #${variation.id}`; addToCartCustom({ ...product, id: `${product.id}_var_${variation.id}`, product_id: product.id, variation_id: variation.id, name: `${product.name} - ${attr}`, price }); } else { const isOpenAmount = product?.price === '' || product?.price === null || (parseFloat(product?.price || 0) === 0 && !product?.is_fixed_zero); if (isOpenAmount) { const openAmountId = `${product.id}_open_${Number(price || 0).toFixed(2)}`; addToCartCustom({ ...product, id: openAmountId, product_id: product.id, variation_id: 0, price: Number(price || 0), name: product.name, is_open_amount: true }); } else addToCart(product, price); } setStockWarningModal({ show: false, product: null, variation: null, price: 0 }); };
  const addToCart = (product, overridePrice = null) => { const finalPrice = overridePrice !== null ? overridePrice : parseFloat(product.price || 0); setCart(prev => { const existing = prev.find(item => item.id === product.id); if (existing) return prev.map(item => item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item); return [...prev, { ...product, price: finalPrice, quantity: 1, product_id: product.id, variation_id: 0 }]; }); };
  const addToCartCustom = cartItem => { setCart(prev => { const existing = prev.find(item => item.id === cartItem.id); if (existing) return prev.map(item => item.id === cartItem.id ? { ...item, quantity: item.quantity + 1 } : item); return [...prev, { ...cartItem, quantity: 1 }]; }); };
