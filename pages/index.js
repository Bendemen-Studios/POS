...
export default function POSHome() {
  ...
  const handleSyncData = async () => { if (isSyncing) return; setIsSyncing(true); try { await Promise.allSettled([fetchProducts(), fetchCustomers(), fetchUsersAsCustomers(), fetchPickupOrders(), fetchStores()]); await triggerOfflineSync(false); } finally { setIsSyncing(false); } };
  const fetchProducts = async () => { try { const res = await fetchWithServerCheck(`/api/woocommerce/products?_sync=${Date.now()}`, {}, 10000); const data = await res.json(); if (data.success && data.products) { setProducts(data.products); localStorage.setItem('pos_cached_products', JSON.stringify(data.products)); localStorage.setItem('admin_products', JSON.stringify(data.products)); localStorage.setItem('pos_cached_products_updated_at', String(Date.now())); localStorage.setItem('admin_products', JSON.stringify(data.products)); window.dispatchEvent(new CustomEvent('pos:inventory-synced', { detail: { products: data.products } })); } } catch (err) { console.error('Fout bij ophalen producten:', err); } };
  ...
  const fetchPickupOrders = async () => { try { setLoadingPickup(true); const res = await fetchWithServerCheck('/api/woocommerce/pickup-order'); const data = await res.json(); if (data.success) { const orders = Array.isArray(data.orders) ? data.orders : []; setPickupOrders(orders); localStorage.setItem('pos_cached_pickup_orders', JSON.stringify(orders)); } } catch (err) { console.error('Fout bij ophalen afhaalbestellingen:', err); } finally { setLoadingPickup(false); } };
