...
export default function POSHome() {
  ...
  const [serverOnline, setServerOnline] = useState(() => typeof window !== 'undefined' ? localStorage.getItem('pos_server_online') !== '0' : true);
  ...
  useEffect(() => {
    if (products.length) localStorage.setItem('pos_cached_products', JSON.stringify(products));
  }, [products]);
  useEffect(() => {
    if (customers.length) localStorage.setItem('pos_cached_customers', JSON.stringify(customers));
  }, [customers]);
  ...
  useEffect(() => localStorage.setItem('pos_active_tab', activeTab), [activeTab]);
  ...
  const checkServerConnection = async () => {
    ...
    if (serverCheckState.current.checking) return serverCheckState.current.failures < 2;
    ...
    } catch (_) {} finally { clearTimeout(timer); serverCheckState.current.checking = false; }
  ...
  const fetchWithServerCheck = async (url, options = {}, timeout = 10000) => {
    ...
    try {
      return await fetch(url, { ...options, cache: 'no-store', signal: controller.signal, headers: { ...(options.headers || {}), 'Cache-Control': 'no-cache, no-store, must-revalidate' } });
  ...
  useEffect(() => {
    ...
    // Geen await/sync hier: de POS moet direct openen met de lokale cache.
    setIsChecking(false);
  }, [router]);
  ...
  const triggerOfflineSync = async (isManualClick = false) => {
    ...
    offlineSyncInFlight.current = (async () => {
      try {
        const queue = readLocalArray('pos_offline_orders');
        setPendingOfflineCount(queue.length);
        if (!queue.length) { if (isManualClick) alert('Er staan geen offline bestellingen in de wachtrij.'); return; }
        ...
        for (const order of queue) {
          try {
            ...
            else { lastError = data.error || data.message || `HTTP ${res.status}`; remaining.push(order); }
          } catch (err) { lastError = err.message || 'Geen verbinding met de server'; remaining.push(order); }
        }
        if (remaining.length) localStorage.setItem('pos_offline_orders', JSON.stringify(remaining));
        ...
        if (successCount > 0) {
          ...
          await fetchProducts();
        } else if (remaining.length && isManualClick) {
          const clear = confirm(`⚠️ Synchroniseren van ${remaining.length} offline bestelling(en) mislukt.\n\nFoutmelding van server:\n"${lastError}"\n\nWil je deze vastgelopen offline bestelling(en) WISSEN uit de kassa?`);
          if (clear) { localStorage.removeItem('pos_offline_orders'); setPendingOfflineCount(0); }
        }
      } finally { setIsSyncing(false); offlineSyncInFlight.current = null; }
    })();
  ...
  useEffect(() => {
    ...
    const backgroundSync = async (force = false) => {
      ...
      try {
        // Altijd eerst de offline orderqueue verwerken.
        await triggerOfflineSync(false);
        // Daarna de lokale cache bijwerken zonder de UI te blokkeren.
        if (!stopped && await checkServerConnection()) {
          await Promise.allSettled([fetchProducts(), fetchCustomers(), fetchUsersAsCustomers(), fetchStores()]);
        }
      } catch (err) { console.warn('[AUTO SYNC] achtergrond-sync mislukt:', err); }
      finally { backgroundSyncState.current.running = false; }
    };

    // Eerste sync is bewust na render en blokkeert de kassa niet.
    backgroundSync(true);
    ...
    return () => {
      stopped = true; clearInterval(healthTimer); clearInterval(fullSyncTimer);
      window.removeEventListener('online', online); window.removeEventListener('focus', wake); document.removeEventListener('visibilitychange', wake);
    };
  }, []);
  ...
  const fetchStores = async () => {
    try {
      ...
      const stores = (Array.isArray(data.stores) ? data.stores : data.store ? [data.store] : []).map(s => ({ ...s, payment_methods: parsePaymentMethods(s.payment_methods) }));
      setAllStores(stores); localStorage.setItem('admin_cached_stores', JSON.stringify(stores));
      const saved = localStorage.getItem('selectedStore') || localStorage.getItem('pos_selected_store');
      if (saved) {
        try { const obj = JSON.parse(saved); const match = stores.find(s => String(s.id || s.store_id) === String(obj.id || obj.store_id)); if (match) handleSelectStore(match, false); else if (stores.length) handleSelectStore(stores[0], false); } catch (_) { if (stores.length) handleSelectStore(stores[0], false); }
      } else if (stores.length) handleSelectStore(stores[0], false);
  ...
  const fetchProducts = async () => {
    try {
      ...
      const data = await res.json();
      if (data.success && data.products) { setProducts(data.products); localStorage.setItem('pos_cached_products', JSON.stringify(data.products)); localStorage.setItem('admin_products', JSON.stringify(data.products)); localStorage.setItem('pos_cached_products_updated_at', String(Date.now())); setServerOnline(true); localStorage.setItem('pos_server_online', '1'); }
    } catch (err) { console.error('Fout bij ophalen producten:', err); }
  };

  const fetchCustomers = async () => {
    try { const res = await fetchWithServerCheck('/api/woocommerce/customers'); const data = await res.json(); if (data.success && data.customers) { setCustomers(data.customers); localStorage.setItem('pos_cached_customers', JSON.stringify(data.customers)); } }
  ...
  const fetchUsersAsCustomers = async () => {
    try {
      ...
      if (data.success && data.users) {
        const staff = data.users.map(u => ({ id: `user_${u.id}`, first_name: u.username || '', last_name: u.role ? `(${u.role})` : '(Gebruiker)', email: u.email || `${u.username}@bendemen.local` }));
        setCustomers(prev => { const ids = new Set(prev.map(c => String(c.id))); const merged = [...prev, ...staff.filter(s => !ids.has(String(s.id)))]; localStorage.setItem('pos_cached_customers', JSON.stringify(merged)); return merged; });
      }
    } catch (err) { console.error('Fout bij ophalen gebruikers:', err); }
  };

  const fetchPickupOrders = async () => {
    try { setLoadingPickup(true); const res = await fetchWithServerCheck('/api/woocommerce/pickup-order'); const data = await res.json(); if (data.success) { const orders = Array.isArray(data.orders) ? data.orders : []; setPickupOrders(orders); localStorage.setItem('pos_cached_pickup_orders', JSON.stringify(orders)); } }
  ...
  const getProductCategory = p => { if (p.categories?.length) { const c = p.categories.find(cat => !EXCLUDED_CATEGORIES.includes(cat.name)); if (c) return c.name; } return 'Overige'; };
  ...
  const handleProductClick = product => {
    const vars = Array.isArray(product.variations_data) && product.variations_data.length ? product.variations_data : (Array.isArray(product.variations) && typeof product.variations[0] === 'object' ? product.variations : []);
    if (vars.length) { setSelectedProductForVariations({ ...product, variations_data: vars }); return; }
    const price = parseFloat(product.price || 0);
    if (product.stock_quantity !== null && product.stock_quantity !== undefined && Number(product.stock_quantity) <= 0) { setStockWarningModal({ show: true, product, variation: null, price }); return; }
  ...
  const addToCart = (product, overridePrice = null) => setCart(prev => { const existing = prev.find(i => i.id === product.id); if (existing) return prev.map(i => i.id === product.id ? { ...i, quantity: i.quantity + 1 } : i); return [...prev, { ...product, price: overridePrice !== null ? overridePrice : parseFloat(product.price || 0), quantity: 1, product_id: product.id, variation_id: 0 }]; });
  const addToCartCustom = item => setCart(prev => { const existing = prev.find(i => i.id === item.id); if (existing) return prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i); return [...prev, { ...item, quantity: 1 }]; });
  const updateQuantity = (id, delta) => setCart(prev => prev.map(i => i.id === id ? { ...i, quantity: i.quantity + delta } : i).filter(i => i.quantity > 0));

  const subtotal = cart.reduce((a, i) => a + parseFloat(i.price || 0) * i.quantity, 0);
  ...
  const handleOpenPaymentModal = () => {
    ...
    const valid = (method === 'sumup' && methods.sumup !== false) || (method === 'manual_pin' && methods.manual_pin !== false) || (method === 'cash' && methods.cash !== false);
    if (!valid) method = methods.sumup !== false ? 'sumup' : methods.manual_pin !== false ? 'manual_pin' : 'cash';
    setSelectedPaymentMethod(method); setCashGiven(finalTotal.toFixed(2)); setShowPaymentModal(true);
  };

  const triggerPrintReceipt = async orderData => {
    try { const response = await fetchWithServerCheck('/api/pos/print-receipt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(orderData) }); if (!response.ok) throw new Error(`Printserver HTTP ${response.status}`); const html = await response.text(); const win = window.open('', '_blank', 'width=400,height=600'); if (!win) { alert('Sta pop-ups toe om de bon te kunnen printen.'); return; } win.document.write(html); win.document.close(); win.focus(); setTimeout(() => { try { win.print(); } catch (_) {} }, 250); }
    catch (err) { alert('Bon printen mislukt: ' + (err.message || 'onbekende fout')); }
  };
  ...
  const executeCheckout = async () => {
    ...
    if (selectedPaymentMethod === 'sumup') {
      ...
      catch (err) { alert(`❌ SumUp Betaling Mislukt:\n\n${err.message}\n\nDe bestelling is GEANNULEERD en niet aangemaakt.`); setLoading(false); return; }
    }
    const orderPayload = {
      orderItems: cart, paymentMethod: selectedPaymentMethod, storeId: selectedStore?.id || 1, cashierId: currentUser?.id || 1,
    ...
    try {
      ...
      if (res.ok && data.success) {
        const change = selectedPaymentMethod === 'cash' && changeDue > 0 ? ` (Wisselgeld: €${changeDue.toFixed(2)})` : '';
        setCheckoutStatus({ success: true, message: `Bestelling direct verwerkt in WooCommerce!${change}` });
      } else throw new Error(data.error || `Server retourneerde HTTP ${res.status}`);
    } catch (err) {
      const queue = readLocalArray('pos_offline_orders'); queue.push(orderPayload); localStorage.setItem('pos_offline_orders', JSON.stringify(queue)); setPendingOfflineCount(queue.length);
      const change = selectedPaymentMethod === 'cash' && changeDue > 0 ? ` (Wisselgeld: €${changeDue.toFixed(2)})` : '';
      setCheckoutStatus({ success: true, message: `⚠️ Server niet bereikbaar. Bestelling lokaal opgeslagen en wordt automatisch gesynchroniseerd zodra de server terug is.${change}` });
    } finally {
  ...
  const filteredPickupOrders = pickupOrders.filter(o => !selectedStore?.pickup_id || o.shipping_lines?.some(s => s.meta_data?.some(m => m.key === 'pickup_location_id' && m.value === String(selectedStore.pickup_id))));
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col relative">
      ...
      {showStoreModal && <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-lg p-6 max-w-md w-full space-y-4 shadow-2xl"><h3 className="text-base sm:text-lg font-bold">📍 Koppel Kassasysteem aan Filiaal</h3><div className="space-y-2 max-h-60 overflow-y-auto">{allStores.map(s => <button key={s.id || s.store_id} onClick={() => handleSelectStore(s)} className="w-full p-3 border rounded text-left hover:bg-gray-100 transition flex justify-between items-center"><div><div className="font-bold text-sm">{s.store_name || s.name}</div></div></button>)}</div></div></div>}
