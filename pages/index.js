...
export default function POSHome() {
  ...
  const handleMarkAsPickedUp = async orderId => { if (!confirm(`Weet je zeker dat bestelling #${orderId} is opgehaald? De status wordt gewijzigd naar Voltooid.`)) return; try { const res = await fetchWithServerCheck('/api/woocommerce/pickup-order', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ order_id: orderId, status: 'completed' }) }); const data = await res.json(); if (data.success) { setPickupOrders(prev => prev.filter(o => o.id !== orderId)); alert(`Bestelling #${orderId} succesvol gemarkeerd als opgehaald!`); } else alert('Fout bij bijwerken status: ' + (data.error || 'Onbekende fout')); } catch (_) { alert('Fout bij bijwerken status.'); } };
