  ...
  const fetchUsersAsCustomers = async () => {
    try {
      ...
      if (data.success && data.users) {
        const staffAsCustomers = data.users.map(u => ({ id: `user_${u.id}`, username: u.username || '', user_login: u.username || '', first_name: u.username || '', last_name: u.role ? `(${u.role})` : '(Gebruiker)', email: u.email || `${u.username}@bendemen.local` }));
        setCustomers(prev => {
