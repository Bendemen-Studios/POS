...
export default function POSHome() {
  ...
  const checkServerConnection = async () => {
    ...
    try {
      ...
      // VPS unavailable; local POS must keep working.
    } finally {
      clearTimeout(timer);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (reachable) {
        try { await handleSyncData(); } catch (err) { console.warn('[STARTUP SYNC] sync mislukt:', err); }
      }
      setIsChecking(false);
    };
    initialize();
  ...
  useEffect(() => {
    ...
    const handleOnlineEvent = () => tick();
    const handleWake = () => { if (!document.hidden) tick(); };
    tick();
    const health = setInterval(tick, 15000);
    const fullSync = setInterval(async () => {
      if (await checkServerConnection()) {
        try { await handleSyncData(); } catch (err) { console.warn('[AUTO SYNC] sync mislukt:', err); }
      }
    }, 300000);
    ...
    return () => {
      clearInterval(health); clearInterval(fullSync);
      ...
      if (data.success) {
        ...
        if (storeStr) {
          try {
            ...
            } else if (parsedStores.length) {
              handleSelectStore(parsedStores[0], false);
            }
          } catch (e) {
            if (parsedStores.length) handleSelectStore(parsedStores[0], false);
          }
  ...
  const fetchPickupOrders = async () => {
    try {
      ...
      const res = await fetchWithServerCheck('/api/woocommerce/pickup-order', {}, 30000);
      const data = await res.json();
      if (data.success) {
  ...
  const getProductCategory = (product) => {
    if (product.categories && product.categories.length > 0) {
      const validCategory = product.categories.find(cat => !EXCLUDED_CATEGORIES.includes(cat.name));
  ...
  const handleProductClick = (product) => {
    ...
    if (productVariations.length > 0) {
      setSelectedProductForVariations({ ...product, variations_data: productVariations });
      return;
    }
  ...
  const handleSelectVariation = (variation) => {
    ...
    if (varStock !== null && varStock !== undefined && Number(varStock) <= 0) {
  ...
  const handleRedeemPoints = async () => {
    ...
    try {
      const res = await fetchWithServerCheck('/api/woocommerce/points', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customerId: selectedCustomer.id, pointsToRedeem: pts, action: 'redeem' })
      });
      const data = await res.json();
      if (data.success) setRedeemedDiscount(data.discountAmount);
      else alert(data.message || 'Fout bij inwisselen punten.');
    } catch (e) {
      setRedeemedDiscount((pts * 0.05).toFixed(2));
    }
  };
  ...
  const handleOpenPaymentModal = () => {
    ...
    if (!isCurrentValid) validMethod = methods.sumup !== false ? 'sumup' : methods.manual_pin !== false ? 'manual_pin' : 'cash';
  ...
  const triggerPrintReceipt = async (orderData) => {
    try {
      const response = await fetchWithServerCheck('/api/pos/print-receipt', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const processSumUpPayment = async (amount, storeId) => {
    const res = await fetchWithServerCheck('/api/sumup/proxy?action=pay', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleInitiatePayment = () => {
    ...
    if (selectedPaymentMethod === 'sumup' && finalTotal < 1) {
      alert('❌ SumUp betalingen moeten minimaal €1,00 zijn.');
      return;
    }
  ...
  const executeCheckout = async () => {
    ...
    try {
      ...
      const changeText = selectedPaymentMethod === 'cash' && changeDue > 0 ? ` (Wisselgeld: €${changeDue.toFixed(2)})` : '';
      setCheckoutStatus({ success: true, message: `⚠️ Directe checkout mislukt (${err.message}). Bestelling lokaal opgeslagen en wordt automatisch gesynchroniseerd zodra de server weer bereikbaar is.${changeText}` });
    } finally {
  ...
  if (isChecking || !currentUser) {
    return <div className="min-h-screen bg-black flex flex-col items-center justify-center p-4"><div className="text-center space-y-2"><h1 className="text-white font-black text-xl tracking-wider">BDM POS</h1><div className="text-red-600 font-bold text-xs uppercase tracking-widest animate-pulse">Sessie controleren...</div></div></div>;
  }
      ...
      {activeTab === 'pickup' ? (
        ...
        <div className="flex-1 flex flex-col lg:flex-row p-3 sm:p-4 gap-4 overflow-hidden">
          <div className="w-full lg:w-3/5 flex flex-col bg-white rounded-lg shadow p-3 sm:p-4">
            <div className="flex space-x-2 mb-3"><input type="text" placeholder="Zoek producten op naam..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 p-2.5 sm:p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-black text-sm"/><button onClick={() => setShowCustomModal(true)} className="bg-black hover:bg-gray-800 text-white font-bold px-3 py-2 rounded text-xs whitespace-nowrap transition">+ Custom</button></div>
            <div className="flex space-x-2 overflow-x-auto pb-3 mb-3 border-b no-scrollbar"><button onClick={() => setSelectedCategory('ALL')} className={`px-3 sm:px-4 py-2 rounded text-xs font-bold whitespace-nowrap transition ${selectedCategory === 'ALL' ? 'bg-black text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>📦 Alles ({activeProducts.length})</button>{categoriesList.map(cat => { const count = activeProducts.filter(p => getProductCategory(p) === cat).length; return <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 sm:px-4 py-2 rounded text-xs font-bold whitespace-nowrap transition ${selectedCategory === cat ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>{cat} ({count})</button>; })}</div>
            <div className="flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-3 max-h-[calc(100vh-280px)]">{filteredProducts.map(product => { const imageUrl = product.images?.length ? product.images[0].src : null; const vars = Array.isArray(product.variations_data) && product.variations_data.length ? product.variations_data : (Array.isArray(product.variations) && typeof product.variations[0] === 'object' ? product.variations : []); const zero = parseFloat(product.price || 0) === 0; const stock = product.stock_quantity; return <div key={product.id} onClick={() => handleProductClick(product)} className="bg-gray-50 border border-gray-200 rounded-lg p-2 flex flex-col justify-between cursor-pointer hover:border-black transition shadow-sm hover:shadow relative z-0"><div><div className="w-full aspect-square bg-gray-200 rounded mb-2 overflow-hidden flex items-center justify-center relative">{imageUrl ? <img src={imageUrl} alt={product.name} className="w-full h-full object-cover" /> : <span className="text-gray-400 text-xs font-bold">GEEN FOTO</span>}{vars.length > 0 && <span className="absolute top-2 right-2 bg-black text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase z-1 shadow">Variaties</span>}{stock !== null && <span className={`absolute bottom-1 right-1 text-[10px] px-1.5 py-0.5 rounded font-bold ${stock <= 0 ? 'bg-red-600 text-white' : 'bg-black text-white'}`}>{stock}</span>}</div><h3 className="font-semibold text-xs line-clamp-2">{product.name}</h3></div><div className="mt-2 flex justify-between items-center"><span className="text-[10px] text-gray-500 font-bold uppercase truncate max-w-[70px]">{getProductCategory(product)}</span><span className="font-bold text-xs sm:text-sm text-red-600">{zero ? 'Open' : `€${parseFloat(product.price || 0).toFixed(2)}`}</span></div></div>; })}</div>
          </div>

          <div className="w-full lg:w-2/5 flex flex-col bg-white rounded-lg shadow p-3 sm:p-4 justify-between">
            <div><h2 className="text-base sm:text-lg font-bold mb-3 border-b pb-2">Huidige Bestelling</h2>
