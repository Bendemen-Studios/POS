...
for env_file in .env .env.local; do
  if [ -f "$env_file" ]; then cp -f "$env_file" "$ENV_BACKUP.$(basename "$env_file")"; fi
done
...
for env_file in .env .env.local; do
  ...
  if [ -f "$backup" ]; then cp -f "$backup" "$env_file"; rm -f "$backup"; fi
done

# Keep the active POS checkout/queue behavior consistent on every deployment.
...
# Keep the service worker timeout/version synchronized with the POS checkout.
...
s = s.replace("timeoutFetch(request, 1500)", "timeoutFetch(request, 5000)")
if s != original:
    path.write_text(s, encoding='utf-8')
    print('✅ Service worker patched: v22 + 45s checkout timeout + 5s health timeout.')
else:
    print('ℹ️ Service worker already patched; no change applied.')
PY
else
...
if [ ! -f package.json ]; then
  echo "❌ package.json ontbreekt na git update. Deployment gestopt."; exit 1
fi
...
if pm2 describe "$APP_NAME" >/dev/null 2>&1; then
  ...
  if [ -f ecosystem.config.js ]; then pm2 start ecosystem.config.js --update-env; else pm2 start npm --name "$APP_NAME" -- start; fi
fi
...
if command -v curl >/dev/null 2>&1 && [ -f "$PRELOAD_SCRIPT" ]; then
  ...
  if "$PRELOAD_SCRIPT"; then echo "✅ VPS productcache is voorgeladen."; else echo "⚠️ VPS preload kon nog niet worden uitgevoerd; cron probeert dit opnieuw."; fi
else
