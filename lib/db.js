...
export async function initDatabase() {
  ...
  try {
    ...
    // Existing installations can have a legacy `users` table with only a
    // subset of the current columns. Add every column used by the active APIs
    // before migrations/login so dbReady cannot fail on an older schema.
    const userColumns = [
      ['email', 'VARCHAR(255) DEFAULT NULL'],
      ['password', 'VARCHAR(255) DEFAULT NULL'],
      ['password_hash', 'VARCHAR(255) DEFAULT NULL'],
...
// API routes can await this on a cold start so schema creation/migrations are
// finished before their first database query.
