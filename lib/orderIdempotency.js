...
export async function claimOrder(clientOrderId) {
  ...
  // A failed/crashed request must not block the POS queue for 10 minutes.
  // Normal checkout calls finish well before this timeout; after 30 seconds
  // an abandoned processing lock is safe to retry.
  const stale = row.status === 'processing' &&
    Date.now() - new Date(row.updated_at).getTime() > 30 * 1000;
