...
function getPointsMeta(customer) {
  ...
  // WooCommerce/plugins can leave an old 0-valued field beside the real balance.
  // Use the highest valid balance so a real 100-point balance is never hidden by a stale 0.
...
export function calculateEarnedPoints(totalPaid) {
  ...
  return Number.isFinite(euros) && euros > 0 ? Math.floor(euros) : 0;
}
