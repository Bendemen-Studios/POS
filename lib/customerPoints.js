...
function getWooConfig() {
  ...
  const consumerKey = process.env.WOOCOMMERCE_CONSUMER_KEY || process.env.WOOCOMMERCE_KEY || process.env.NEXT_PUBLIC_WOOCOMMERCE_KEY || process.env.WOO_CONSUMER_KEY;
  const consumerSecret = process.env.WOOCOMMERCE_CONSUMER_SECRET || process.env.WOOCOMMERCE_SECRET || process.env.NEXT_PUBLIC_WOOCOMMERCE_SECRET || process.env.WOO_CONSUMER_SECRET;
...
function getPointsMeta(customer) {
  ...
  const keys = new Set(['wc_points_balance', 'points_balance', 'points']);
  const entry = meta.find((item) => keys.has(String(item?.key || '').trim().toLowerCase()));
  const raw = entry?.value ?? customer?.points_balance ?? customer?.points ?? 0;
  const points = parseInt(String(raw).replace(/[^0-9-]/g, ''), 10);
...
export async function updateCustomerPoints({ customerId, pointsUsed = 0, totalPaid = 0 }) {
  ...
  const pointKeys = new Set(['wc_points_balance', 'points_balance', 'points']);
  const existingIndex = metaData.findIndex((item) => pointKeys.has(String(item?.key || '').trim().toLowerCase()));
  const targetKey = existingIndex >= 0 ? metaData[existingIndex].key : 'wc_points_balance';
  const pointsMeta = { key: targetKey, value: String(newBalance) };

  if (existingIndex >= 0) metaData[existingIndex] = { ...metaData[existingIndex], ...pointsMeta };
