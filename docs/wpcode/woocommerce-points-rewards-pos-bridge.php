...
function bdm_pos_points_permission( WP_REST_Request $request ) {
    if ( ! class_exists( 'WC_Points_Rewards_Manager' ) ) {
        return new WP_Error('bdm_points_rewards_missing', 'WooCommerce Points & Rewards is not active.', array('status' => 503));
    }

    // WooCommerce REST API keys inherit the capabilities of their linked
    // WordPress user. Do not require the broader manage_woocommerce capability
    // here: a valid POS REST key may legitimately be a shop-manager/customer
    // management key without being a settings administrator.
    ...
    if ( ! $first_customer_id && is_string( $customer_ids_raw ) ) {
        $first = trim( explode(',', $customer_ids_raw)[0] ?? '' );
        $first_customer_id = absint( $first );
    }

    $context = $request->get_method() === 'GET' ? 'read' : 'edit';
    if ( function_exists( 'wc_rest_check_user_permissions' ) && $first_customer_id > 0 ) {
        if ( wc_rest_check_user_permissions( $context, $first_customer_id ) ) return true;
    }
...
function bdm_pos_points_balance( WP_REST_Request $request ) {
    ...
    if (count($balances) === 1) {
        ...
        return rest_ensure_response(array(
            ...
            'source' => 'woocommerce-points-and-rewards',
        ));
    }
...
function bdm_pos_points_mutation( WP_REST_Request $request ) {
    ...
    if ($already_redeemed > 0) return rest_ensure_response(array(
        'success' => true,
        'idempotent' => true,
    ...
    try {
        ...
        $result = WC_Points_Rewards_Manager::decrease_points(
            ...
            $points,
            'bdm-pos-redeem',
            array('source' => 'bendemen-pos', 'order_id' => $order_id, 'points_redeemed' => $points),
            $order_id
        );
